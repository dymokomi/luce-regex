...
tables (413 compile rows, 601 exec rows and 132 escape rows, plus 97 compile and 376 exec
rows) were generated by running upstream's C locally; only the generated Luce tables are kept
here, the C drivers stay outside the repository. Two cases (four rows) differ on purpose, documented in
